"""Projected state — the git ref a trial validates against (RFC §5.1).

This is the only thing that differs between strategies. M1 ships optimistic and serial:

    optimistic → master HEAD
    serial     → master + the single predecessor ahead in the partition line

Speculative (master + all in-flight predecessors up to depth) and batched are M5/M6.
"""
...
class ProjectedState:
    ...
    predecessor_shas: list[str]  # in-flight predecessors folded into the projection (RFC §5.1)
...
def _strictly_ahead(slot: Slot) -> Q:
...
def build(slot: Slot, strategy: Strategy, *, master_head_sha: str) -> ProjectedState:
    ...
    raise ValueError(f"strategy not supported in M1: {strategy}")
