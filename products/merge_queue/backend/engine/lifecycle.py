"""Engine lifecycle — the enroll → trial → merge/eject state machine (RFC §5).
...
Side effects on GitHub (the actual merge) go through the module `ShadowGuard`, which in M1 is
shadow — it records, it does not act.
"""
...
_shadow = ShadowGuard(github=None)  # M1: shadow — record, don't act
