"""Trial-workflow activities (RFC §3).
...
`run_full_suite` is the injectable CI-runner seam. Real PostHog-CI dispatch lands in M4 (the
affected-target graph + `ci/scoping.py` full-suite-vs-subset split); M1 ships the seam as a
stub so the orchestration is in place and testable. The full suite — no test selection — is
the §5.2 correctness invariant and is never configurable here.
"""
...
def run_full_suite(ref: TrialRef) -> SuiteResult:
    ...
    TODO(M4): dispatch the real full-suite CI run (affected-target scope for the partition,
    ...
    """
    raise NotImplementedError("full-suite CI dispatch lands in M4; inject a runner in tests")
