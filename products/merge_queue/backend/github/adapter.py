...
facade. Outbound: merges and commit statuses, routed through the engine's `ShadowGuard` (M1 is
shadow, so they record and do not act).
...
wiring are surfaces work (M2); M1 implements the ingest/eligibility/enroll core and the
outbound seam.
"""
...
# Queue-admission predicate (RFC §6). Partition predicates layer on top once the router lands (M3).
...
class PullRequestSignal:
    ...
    required_checks: list[str] = field(default_factory=list)  # incl. the Visual Review gate (§7.1)
...
class GitHubAdapterOut(GitHubOut):
    ...
    Only used once a partition is promoted out of shadow (M2). In M1 the engine holds a
    ...
    def merge(self, repo: str, number: int, sha: str) -> None:
        raise NotImplementedError("live merge lands with hybrid/exclusive promotion (M2)")

    def set_status(self, repo: str, sha: str, *, state: str, context: str, description: str) -> None:
        raise NotImplementedError("live commit-status lands with hybrid/exclusive promotion (M2)")


def install_engine_bindings(*, integration_id: int) -> None:
    """Wire the engine's external seams to this GitHub integration (called by the M2 webhook setup)."""
