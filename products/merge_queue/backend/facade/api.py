"""Imperative facade surface (locked — `docs/stampede-facade-and-data-model.md` B.2).
...
from products.merge_queue.backend.engine.projected_state import _strictly_ahead
...
def enroll(pr: PRRef, *, actor: Actor, approval_ref: str = "") -> EnrollmentStatus:
    ...
    # M1 routes to a single partition; partition-spanning routing (multi-slot) is M3.
    enrollment = lifecycle.enroll_pr(pr, actor=actor, partition=partitions[0], approval_ref=approval_ref)
    status_ = _to_status(enrollment)
...
def dequeue(pr: PRRef, *, actor: Actor, reason: str) -> None:
    ...
    partition = enrollment.slots.first().partition if enrollment.slots.exists() else None
...
def break_glass(pr: PRRef, *, actor: Actor) -> None:
    ...
    enrollment = _active_enrollment(pr.repo, pr.number)
    partition = enrollment.slots.first().partition if (enrollment and enrollment.slots.exists()) else None
    # shadow-guarded forced merge — records, does not act in M1
...
def _resolve_partitions(pr: PRRef) -> list[Partition]:
    """M1: route to a single partition (lowest precedence). The predicate-matching router
    that computes the full safe set (and partition-spanning PRs) is M3."""
...
def _position(slot: Slot) -> int:
    return (
        ...
        .filter(_strictly_ahead(slot))
        .count()
    )


def _to_status(enrollment: Enrollment) -> EnrollmentStatus | None:
    if enrollment is None:
        return None
    slots = [
        SlotStatus(
    ...
    return EnrollmentStatus(
        ...
        blocked_by=None,  # unlanded stack parent — stacks are surfaced from M3+
