"""Observability — the single QueueEvent emit point (RFC §13).
...
def _to_engineering_analytics(event: QueueEvent) -> None:
    ...
    does not exist yet (RFC §13, §18). Lean is `pull_request`-group-typed events through the
...
async def record_shadow(hook: str, args: tuple, kwargs: dict, taken: Any) -> None:
    """Record a Cowboy would-be decision as a SHADOW_DECISION event (RFC §4.2).

    Wired into `GatedProvider`; dormant in M1 because `cowboy=None`. `would_be` (the actual
    Cowboy call) is filled in when Cowboy lands (M7); for now we capture the hook and the
