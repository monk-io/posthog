...
class ExternalDataSourceViewSet(TeamAndOrgViewSetMixin, AccessControlViewSetMixin, viewsets.ModelViewSet):
    ...
    def disable_cdc(self, request: Request, *arg: Any, **kwargs: Any):
        ...
        with transaction.atomic():
            ...
            # Clear any broken marker (recovery contract) atomically with the reset above: a stale
            # cdc_broken left in sync_type_config would make CDC look broken the moment it's re-enabled.
