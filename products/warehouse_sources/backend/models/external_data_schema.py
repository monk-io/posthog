...
class ExternalDataSchema(ModelActivityMixin, CreatedMetaFields, UpdatedMetaFields, UUIDTModel, DeletedMetaFields):
    ...
    # deterministic one-way SHA-256 digest salted by team_id (for sensitive data: passwords, PII, etc.). PK
    # columns and the active incremental field can't be masked — that would corrupt merges and the cursor.
