...
class TestMCPToolNeighborsQueryRunner(_MCPAnalyticsTeamScopedTestMixin, ClickhouseTestMixin, APIBaseTest):
    def _run(self, direction: Direction, tool_name: str = "query_run") -> list[Any]:
        runner = MCPToolNeighborsQueryRunner(
            query=MCPToolNeighborsQuery(toolName=tool_name, direction=direction, dateRange=DateRange(date_from="-7d")),
            team=self.team,
        )
        return runner.calculate().results
    ...
    @parameterized.expand(
        [
            ("before", Direction.BEFORE, "tool_a"),
            ("after", Direction.AFTER, "tool_b"),
        ]
    )
    def test_finds_adjacent_tool_in_conversation(
        self, _name: str, direction: Direction, expected_neighbor: str
    ) -> None:
        now = datetime.now(tz=UTC)
