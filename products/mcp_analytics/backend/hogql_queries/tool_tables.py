...
class MCPToolStatsQueryRunner(AnalyticsQueryRunner[MCPToolStatsQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        return parse_select(
            ...
            WHERE {{where}}
            """,
            placeholders={"where": _tool_call_where(self.query.toolName, self.query_date_range)},
        )
...
class MCPToolSampleIntentsQueryRunner(AnalyticsQueryRunner[MCPToolSampleIntentsQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        return parse_select(
            ...
            LIMIT 5
            """,
            placeholders={"token": parse_expr(mcp_harness.HARNESS_TOKEN_SQL), "where": where},
        )
...
_WINDOW_FN: dict[Direction, str] = {Direction.BEFORE: "lagInFrame", Direction.AFTER: "leadInFrame"}
_CONV_ID = "coalesce(nullIf(toString(properties.$mcp_session_id), ''), toString(properties.$session_id))"
...
class MCPToolNeighborsQueryRunner(AnalyticsQueryRunner[MCPToolNeighborsQueryResponse]):
    ...
    def to_query(self) -> ast.SelectQuery | ast.SelectSetQuery:
        ...
        window_fn = _WINDOW_FN[self.query.direction]
        cte_where = ast.And(
            exprs=[
                parse_expr("event = {event}", placeholders={"event": ast.Constant(value=MCP_TOOL_CALL_EVENT)}),
                parse_expr(
        ...
        return parse_select(
            ...
            LIMIT 5
            """,
            placeholders={"cte_where": cte_where, "tool": ast.Constant(value=self.query.toolName)},
        )
